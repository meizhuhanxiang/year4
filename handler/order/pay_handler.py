...
class PayHandler(BaseHandler):
    ...
    def post(self):
        ...
        order_model = self.model_config.first(OrderModel, user_id=user_model.id, status=OrderModel.STATUS_WAIT_SEND)  # type: OrderModel
        if order_model:
            raise Exception('用户已购买过该商品')
        order_model = OrderModel(user_id=user_model.id, status=OrderModel.STATUS_WAIT_PAY, out_trade_no=out_trade_no)
        price = self.get_json_argument('price')
